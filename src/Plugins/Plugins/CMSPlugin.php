...
class CMSPlugin extends AbstractPlugin {
    public function activate() {
        try {
            echo "CMS Plugin activated" . PHP_EOL;
            return true;
        } catch (\Exception $e) {
            echo "Error during CMS activation: " . $e->getMessage() . PHP_EOL;
            return false;
        }
    }
    ...
    public function deactivate() {
        try {
            echo "CMS Plugin deactivated" . PHP_EOL;
            return true;
        } catch (\Exception $e) {
            echo "Error during CMS deactivation: " . $e->getMessage() . PHP_EOL;
            return false;
        }
    }
    ...
    public function log($message, $level = 'info') {
        ...
        if (!$this->manager) {
            throw new \Exception("Plugin manager not set for CMSPlugin.");
        }
        $logPath = $this->manager->getLogPath();
        $logFile = $logPath . 'CMS.log';
